...
def p_ar_push(p):
    """
    statement : AR_PUSH ARRAY_VAR
    """
    p[0] = p[1:]

def p_ar_pop(p):
    """
    statement : AR_POP ARRAY_VAR
    """
    p[0] = p[1:]
...
def execute_bad_instruction(instruction):
    ...
    if command == "push" or command == "ar_push":
        a = instruction[1].get_value()
...
def run_code_from_string(input_, ugly_mode, debug_mode, profile_mode):
    global DEBUG_MODE
    DEBUG_MODE = debug_mode
    global UGLY_MODE
    UGLY_MODE = ugly_mode
    return run(input_, profile_mode)

def run_bad_code_from_string(input_, debug_mode=False, profile_mode=False):
    return run_code_from_string(input_,
                                ugly_mode=False,
                                debug_mode=debug_mode,
                                profile_mode=profile_mode)


def run_ugly_code_from_string(input_, debug_mode=False, profile_mode=False):
    return run_code_from_string(input_,
                                ugly_mode=True,
                                debug_mode=debug_mode,
                                profile_mode=profile_mode)
...
def main():
    ...
    if args.ugly:
        target = run_ugly_code_from_string(source, args.debug, args.profile)
    else:
        target = run_bad_code_from_string(source, args.debug, args.profile)
