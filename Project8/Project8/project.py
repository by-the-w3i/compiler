...
class BlockNode(Node):
    ...
    def generate_bad_code(self, output):
        ...
        for child in self.children:
            if not isinstance(child, FuncDefineNode):
...
class FuncCallNode(Node):
    def __init__(self, name, args):
        ...
        self.paras = FUNC_TABLE[self.name].paras
        ...
        if self.register == "x":
            ...
            for i in range(len(self.args)):
                ...
                if "array" in FUNC_TABLE[self.name].paras[i].type:
                    cmd = "ar_copy"
                loadArgs_out.append("{} {} {}".format(cmd, frm, to))

            #### append recurtion push
            if PUPO_LST!=None:
                for p in FUNC_TABLE[self.name].paras:
                    output.append(push(p.register))

                for rg in PUPO_LST:
                    output.append(push(rg))

            # load args to function paras
            output+=loadArgs_out
            ...
            #### pop back
            if PUPO_LST!=None:
                for rg in PUPO_LST[::-1]:
                    output.append(pop(rg))

                for p in FUNC_TABLE[self.name].paras[::-1]:
                    output.append(pop(p.register))
...
def p_func_signature(p):
    ...
    FUNC_LST.append(name) # check duplicate
...
def generate_bad_code_from_string(input_, optimize_mode=False):
    ...
    lexer = lex.lex()
    parser = yacc.yacc()
    # update all the functable
    header(input_)
    dy_s = S_CNT ## dynamic s var starts
    # print("SCNT:", dy_s)
    program = parser.parse(input_, lexer=lexer)
    output = []
    program.generate_bad_code(output)

    #complie the define_functions
    for k in FUNC_TABLE:
        PUPO_LST = []
        FUNC_TABLE[k].generate_bad_code(output)

    #### turn on the optimize_mode
    while optimize_mode:
        optimize_mode = optimize_bad(output, dy_s)
        output = [line for line in output if line != "nop"]
    ####
    return "\n".join(output) + "\n"


def optimize_bad(output, dy_s):
    blkID = 0
    lineNO = 0
    varTB = {}
    changed = False
    ...
    for line in output:
        ...
        # update to the var Table
        if len(lst)==2:
            if "pop" in lst[0]:
                updateVarTB(varTB, lst[1], (blkID, lineNO, cnt), "W")
            else:
                updateVarTB(varTB, lst[1], (blkID, lineNO, cnt), "R")

        if len(lst)==3:
            first = lst[1]
            second = lst[2]
            updateVarTB(varTB, first, (blkID, lineNO, cnt), "R")
            if "jump" in lst[0] or "set" in lst[0]:
                updateVarTB(varTB, second, (blkID, lineNO, cnt), "R")
            else:
                updateVarTB(varTB, second, (blkID, lineNO, cnt), "W")
        ...
        if len(lst)==4:
            ...
            updateVarTB(varTB, second, (blkID, lineNO, cnt), "R")
            if "set" in lst[0]:
                updateVarTB(varTB, third, (blkID, lineNO, cnt), "R")
            else:
                updateVarTB(varTB, third, (blkID, lineNO, cnt), "W")

        cnt += 1
        lineNO += 1

        # update the block ID
        if "jump" in lst[0] or lst[0][-1] == ":":
            blkID += 1
            lineNO = 0
    ...
    for var in varTB:
        if int(var[1:]) < dy_s:
            continue
        if len(varTB[var]["W"]) > 0:
            firstW = varTB[var]["W"][0]
            idx = firstW[2]
            src_cmds = output[idx].strip().split()
            # Dead-code elimination
            used = False
            for rtup in varTB[var]["R"]:
                if rtup > firstW:
                    used = True
                    break
            if not used:
            ...
            if "val_copy" == src_cmds[0]:
                if not check_bad_var(src_cmds[1]):
                    const = src_cmds[1]
                    for rtup in varTB[var]["R"]:
                        if rtup[0]==firstW[0] and rtup > firstW:
                            change_idx = rtup[-1]
                            change_cmds = output[change_idx].strip().split()
                            for i in range(1, len(change_cmds)):
                                if change_cmds[i] == var:
                                    change_cmds[i] = const
                            new_str = " ".join(change_cmds)
                            output[change_idx] = new_str
                            changed = True

            # Copy propagation
            if "copy" in src_cmds[0]:
                frm = src_cmds[1]
                to = src_cmds[2]
                if check_bad_var(frm) and check_bad_var(to):
                    # last use frm == first use to
                    if len(varTB[to]["R"])>0 and varTB[to]["R"][-1][0] == varTB[to]["W"][0][0]:
                        nextW =()
                        if len(varTB[to]["W"]) > 1:
                            nextW = varTB[to]["W"][1]
                        for rtup in varTB[to]["R"]:
                            # if rtup[0] == varTB[to]["W"][0][0]: # same scope
                            if rtup[0] == varTB[to]["W"][0][0] and varTB[to]["W"][0] < rtup and (nextW==() or rtup < nextW):
                                change_idx = rtup[-1]
                                change_cmds = output[change_idx].strip().split()
                                for i in range(1, len(change_cmds)):
                                    if change_cmds[i] == to:
            ...
            if "test" in src_cmds[0] or src_cmds[0] in ["add", "sub", "mult", "div"]:
                p1 = src_cmds[1]
                p2 = src_cmds[2]
                p3 = src_cmds[3]
                cmd = src_cmds[0]
                ...
                if cmd == "add":
                    if (not check_bad_var(p1)) and (not check_bad_var(p2)):
                        output[idx] = "val_copy {} {}".format(int(p1)+int(p2), p3)
                        changed = True
                    else:
                        if not check_bad_var(p1) and int(p1) == 0:
                            output[idx] = "val_copy {} {}".format(p2, p3)
                            changed = True
                        elif not check_bad_var(p2) and int(p2) == 0:
                            output[idx] = "val_copy {} {}".format(p1, p3)
                            changed = True
                    ...
                    if (not check_bad_var(p1)) and (not check_bad_var(p2)):
                        ...
                        if not check_bad_var(p1) and int(p1) == 1:
                            output[idx] = "val_copy {} {}".format(p2, p3)
                            changed = True
                        elif not check_bad_var(p2) and int(p2) == 1:
                            output[idx] = "val_copy {} {}".format(p1, p3)
                            changed = True
                        elif (not check_bad_var(p1) and int(p1) == 0) or (not check_bad_var(p2) and int(p2) == 0):
                            output[idx] = "val_copy 0 {}".format(p3)
                            changed = True
                    ...
                    if (not check_bad_var(p1)) and (not check_bad_var(p2)):
                        ...
                        changed = True
                        if cmd == "test_less":
                            output[idx] = "val_copy {} {}".format(1 if i1 < i2 else 0, p3)
                        elif cmd == "test_gtr":
                            output[idx] = "val_copy {} {}".format(1 if i1 > i2 else 0, p3)
                        elif cmd == "test_equ":
                            output[idx] = "val_copy {} {}".format(1 if i1 == i2 else 0, p3)
...
def updateVarTB(varTB, var, sign, mode):
    if not check_bad_var(var):
        return
    if var not in varTB:
        varTB[var] = {"R":[], "W":[]}
    varTB[var][mode].append(sign)
...
def generate_ugly_code_from_string(input_, debug_mode=False, optimize_mode=False):
    global ARR_COPY_NUM
    result = generate_bad_code_from_string(input_, optimize_mode)
    ...
    for line in result.strip().split("\n"):
        ...
        if len(lst) > 0:
            ...
            if cmd == "val_copy":
                ...
                if "return_call" in lst[1]:
                    output.append("val_copy {} regA".format(lst[1]))
                else:
                    output.append("load {} regA".format(lst[1][1:]))
    ...
    return "\n".join(output) + "\n"
#################################################
### get all the function signature
def header(src):
    import re
    global FUNC_TABLE
    ...
    for tup in result:
        ...
        for p in paras_lst:
            ...
            if p != '':
                ...
                p_type = p_lst[0]
                p_name = p_lst[1]
                para = ParameterNode(p_name, p_type)
                paras.append(para)
...
if __name__ == "__main__":
    ...
    result = generate_bad_code_from_string(source, True)
